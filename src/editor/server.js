...
async function handleApi(req, res, url) {
  ...
  switch (key) {
    case 'GET /api/state': {
      const sites = siteSummaries(workspace)
      if (!site) return json(res, 200, { sites, site: null })
      return json(res, 200, { sites, ...stateOf(workspace, site) })
    }
    case 'GET /api/doc':
...
const defaultSite = requested ?? (sites.length === 1 ? sites[0].id : null)
...
if (!existsSync(join(distDir, 'ui.js'))) {
...
function listen(port, attempts = 20) {
  ...
  server.listen(port, '127.0.0.1', () => {
    const site = siteSummaries(workspace).find((entry) => entry.id === defaultSite)
    console.log('kebab editor')
    console.log(`  站点：${defaultSite} — ${site?.title ?? ''}`)
