...
**图片放大**：doc 页正文里的图片点开看大图——原尺寸居中，宽度上限 92vw；整页与手机上的长截图不缩进视口（缩了就比正文里还小），而是在放大层里上下滚。点图周围的空白处或按 Esc 退出。整套走 HTML 的 `popover`，产物里依然没有一行运行时脚本；已经在链接里的图（`[![说明](图)](地址)`）不包按钮，免得跟跳转抢同一次点击。proto 页 iframe 里的图不在此列。见 [ADR-0012](adr/0012-image-zoom-without-script.md)。

**导航树收起**：导航头部右侧一颗开关把左侧导航收成 44px 窄轨（再点一次摊开，收起后开关落在窄轨里）。开关是一颗排在页面骨架前面的复选框：`.kebab-nav-toggle:checked ~ .kebab-shell` 在骨架上改一个自定义属性，导航宽度与开关位置都由它推出来，再把品牌与导航树 `display: none` 藏掉。产物里依然没有脚本；收起状态不跨页保存 —— 换一页就是换一份 HTML，见 [ADR-0013](adr/0013-nav-collapse.md)。每个站点页面（含概览页）都带它，根门户页没有导航树，不带。

**正文栏宽度**：doc 页与站点概览页的正文栏跟着窗口走，上限 1600px —— 右侧不再挂一大块空白，比正文栏窄的截图因此按原尺寸显示；上限只拦超宽屏，免得一行拉到读不下去。见 [ADR-0014](adr/0014-doc-column-width.md)。
